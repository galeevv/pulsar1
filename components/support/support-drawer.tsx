...
export function SupportDrawer() {
  ...
  const loadTicketDetail = useCallback(async (ticketId: number) => {
    ...
    try {
      const payload = await requestJson<UserSupportTicketDetailResponse>(`/api/support/tickets/${ticketId}`)
  ...
  async function handleCreateTicket(payload: { category: string; message: string; subject: string }) {
  ...
  function renderContent() {
    if (view === "create") {
      return <SupportTicketCreateForm isSubmitting={createSubmitting} onSubmit={handleCreateTicket} />
    }
  ...
  return (
    <Drawer onOpenChange={handleOpenChange} open={open}>
      ...
      <DrawerContent className="before:shadow-none">
        <DrawerHeader className="space-y-2 text-left">
          <div className="flex items-start justify-between gap-3">
            <div className="space-y-1">
              <DrawerTitle>Поддержка</DrawerTitle>
              <DrawerDescription>{headerText}</DrawerDescription>
            </div>
        ...
        <div className="max-h-[68svh] overflow-y-auto px-4 pb-4">{renderContent()}</div>
      </DrawerContent>
    </Drawer>
  )
}
